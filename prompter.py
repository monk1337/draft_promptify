...
class Prompter:
    ...
    def get_multiple_template_variables(self, dict_templates: dict):
        ...
        for key, value in dict_templates.items():
            results[key] = self.get_template_variables(value['environment'], value['template_name'])
        return results
    ...
    def generate_prompt(self, text_input, **kwargs) -> str:
        ...
        variables = self.get_template_variables(loader['environment'], loader['template_name'])
    ...
    def raw_fit(self, prompt: str):
        outputs = [self.model.model_output_raw(output) for output in self.model.run(prompts=[prompt])]
        return outputs
